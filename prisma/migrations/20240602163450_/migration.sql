/*
  ...
  - You are about to drop the column `quiz_id` on the `ChoiceStep` table. All the data in the column will be lost.
  ...
  - Added the required column `question_id` to the `ChoiceStep` table without a default value. This is not possible if the table is not empty.
...
*/
-- DropForeignKey
ALTER TABLE "ChoiceStep" DROP CONSTRAINT "Choice_quiz_id_fkey";
...
-- AlterTable
ALTER TABLE "ChoiceStep" DROP COLUMN "quiz_id",
...
-- AddForeignKey
ALTER TABLE "ChoiceStep" ADD CONSTRAINT "Choice_question_id_fkey" FOREIGN KEY ("question_id") REFERENCES "Question"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
